...
describe("leitura do texto da publicacao", () => {
  it("acha o prazo em numero", () => {
    expect(detectarPrazo("Fica intimado para, no prazo de 15 dias, manifestar-se.")).toBe(15);
    expect(detectarPrazo("apresentar contestacao em 5 (cinco) dias uteis")).toBe(5);
  });
  ...
  it("ignora numero que nao e prazo", () => {
    expect(detectarPrazo("processo 0001234-56.2026.8.26.0100 distribuido")).toBeNull();
    expect(detectarPrazo("valor de R$ 1.500,00")).toBeNull();
  });

  it("marca urgencia por ato, mesmo sem prazo", () => {
    expect(ehUrgente("Designada audiencia de instrucao para 10/10", null)).toBe(true);
  ...
  it("numero de processo: aceita so o formato CNJ", () => {
    expect(normalizarNumeroProcesso("0001234-56.2026.8.26.0100")).toBe("00012345620268260100");
  ...
  it("formata o numero de volta para leitura humana", () => {
    expect(formatarNumeroProcesso("00012345620268260100")).toBe("0001234-56.2026.8.26.0100");
  });
  ...
  it("grava sempre a mesma grafia, digitado com ou sem mascara", () => {
    ...
    expect(numeroParaGravar("0001234-56.2026.8.26.0100")).toBe("00012345620268260100");
    expect(numeroParaGravar(" 00012345620268260100 ")).toBe("00012345620268260100");
  });
...
describe("conversao do item do DJEN", () => {
  ...
  it("descarta item sem id, sem texto ou com data invalida", () => {
    expect(converter({ texto: "x", data_disponibilizacao: "2026-09-10" })).toBeNull();
    expect(converter({ id: 1, data_disponibilizacao: "2026-09-10" })).toBeNull();
    expect(converter({ id: 1, texto: "x", data_disponibilizacao: "nao-e-data" })).toBeNull();
  });
});
...
describe("cliente do DJEN", () => {
  it("manda OAB, UF e periodo na consulta", async () => {
    ...
    responder = (url) => {
      ...
      return { status: 200, json: { items: [comunicacao(1, "Intimacao")], count: 1 } };
    };
  ...
  it("explica o 403 como bloqueio por pais", async () => {
    ...
    await expect(
      buscarPeriodo({ numeroOab: "1", ufOab: "SP", de: new Date(), ate: new Date() })
  ...
  it("percorre as paginas ate completar o total", async () => {
    responder = (url) => {
      ...
      const itens = pagina <= 2 ? [comunicacao(pagina * 10, `pagina ${pagina}`)] : [];
...
d("captura por escritorio", () => {
  beforeAll(async () => {
    ...
    await comEscritorio(alfa, async (db) => {
      await db.oabMonitorada.create({ data: semEscritorio({ numero: "111111", uf: "SP" }) });
    ...
    await comEscritorio(beta, (db) =>
      db.oabMonitorada.create({ data: semEscritorio({ numero: "222222", uf: "MG" }) })
    );
  });
  ...
  it("grava, tria e vincula ao processo ja cadastrado", async () => {
    responder = () => ({
      ...
      json: {
        count: 2,
        items: [
          comunicacao(1001, "Designada audiencia de instrucao. Prazo de 5 dias."),
    ...
    expect(resultado).toMatchObject({ oabsConsultadas: 1, novas: 2, vinculadas: 1 });
  ...
  it("capturar de novo nao duplica", async () => {
    ...
    await expect(comEscritorio(alfa, (db) => db.publicacao.count())).resolves.toBe(2);
  });

  it("a mesma comunicacao por duas OABs entra uma vez so", async () => {
    await comEscritorio(alfa, (db) =>
      db.oabMonitorada.create({ data: semEscritorio({ numero: "333333", uf: "SP" }) })
    );
    responder = () => ({
      status: 200,
      json: { count: 1, items: [comunicacao(2001, "Intimacao compartilhada.")] },
    });
  ...
  it("publicacao de um escritorio nao aparece no outro", async () => {
    await expect(comEscritorio(beta, (db) => db.publicacao.count())).resolves.toBe(0);
  });
  ...
  it("falha de uma OAB nao impede as outras, e nao avanca a marca dela", async () => {
    ...
    responder = (url) => {
      if (url.searchParams.get("numeroOab") === "111111") return { status: 500, json: {} };
      return { status: 200, json: { count: 1, items: [comunicacao(3001, "Outra intimacao.")] } };
    };
