...
function certificadoDeTeste(senha = "senha-do-teste"): string {
  ...
  const nome = [{ name: "commonName", value: "ESCRITORIO TESTE:12345678000199" }];
...
const DADOS: DadosDaDps = {
  ...
  tomador: { documento: "529.982.247-25", nome: "Cliente & Cia", email: "c@teste.br" },
  servico: {
...
describe("formato do XML (formato, nao verdade fiscal)", () => {
  ...
  it("a hora sai no fuso de Brasilia, nao no do servidor", () => {
    ...
    expect(dataHoraXml(new Date("2026-09-19T14:30:00Z"))).toBe("2026-09-19T11:30:00-03:00");
    expect(dataHoraXml(new Date("2026-09-19T02:00:00Z"))).toBe("2026-09-18T23:00:00-03:00");
  });
  ...
  it("cada regime tem um codigo proprio", () => {
    ...
    expect(montarDps(DADOS)).toContain(`<opSimpNac>${OP_SIMPLES.SIMPLES}</opSimpNac>`);
  });
...
describe("assinatura digital (isto e provado de verdade)", () => {
  ...
  it("assina e a assinatura confere", () => {
    ...
    expect(assinado).toContain(certificadoEmBase64(certificado.certificadoPem).slice(0, 40));
  ...
  it("documento adulterado depois de assinado nao confere mais", () => {
    ...
    const certificado = abrirCertificado(pfx, "senha-do-teste");
    const assinado = assinarDps(montarDps(DADOS), certificado, idDaDps(DADOS));
    const adulterado = assinado.replace("<vServ>1500.00</vServ>", "<vServ>15.00</vServ>");

    expect(adulterado).not.toBe(assinado);
    expect(conferirAssinatura(adulterado, certificado.certificadoPem)).toBe(false);
  });

  it("senha errada e arquivo invalido dao mensagem propria", () => {
    expect(() => abrirCertificado(pfx, "senha-errada")).toThrow(CertificadoInvalido);
    expect(() => abrirCertificado("nao-e-um-pfx", "x")).toThrow(CertificadoInvalido);
...
let responder: (caminho: string) => { status: number; json: unknown } = () => ({
  status: 200,
  json: { chaveAcesso: "29260912345678000199", numeroNfse: "1", linkPdf: "https://p/1.pdf" },
});
...
d("emissao por escritorio", () => {
  beforeAll(async () => {
    ...
    for (const id of [alfa, beta]) {
      await comEscritorio(id, (db) =>
        db.moduloContratado.create({ data: semEscritorio({ modulo: "NFSE", ativo: true }) })
      );
    }
    ...
    await comEscritorio(alfa, async (db) => {
      ...
      const sem = await db.cliente.create({ data: semEscritorio({ nome: "Cliente sem CPF" }) });
  ...
  it("emite, guarda o XML assinado e mede o consumo", async () => {
    const antes = (await consumoDoMes(alfa)).find((l) => l.metrica === "NFSE_EMITIDA");
    ...
    const depois = (await consumoDoMes(alfa)).find((l) => l.metrica === "NFSE_EMITIDA");
    expect(depois?.quantidade).toBe((antes?.quantidade ?? 0) + 1);
  });

  it("a numeracao anda de um em um, sem repetir", async () => {
    const primeira = await emitirNota(alfa, { ...pedido, clienteId: clienteAlfa });
    const segunda = await emitirNota(alfa, { ...pedido, clienteId: clienteAlfa });
  ...
  it("recusa da prefeitura fica gravada com o motivo, e o numero nao volta", async () => {
    responder = () => ({
      status: 400,
      json: { erros: [{ codigo: "E0123", descricao: "Inscricao municipal invalida." }] },
    });

    const fiscalAntes = await comEscritorio(alfa, (db) => db.fiscal.findFirst());
    await expect(emitirNota(alfa, { ...pedido, clienteId: clienteAlfa })).rejects.toBeInstanceOf(
      FalhaNaNfse
    );
    const fiscalDepois = await comEscritorio(alfa, (db) => db.fiscal.findFirst());

    const recusada = await comEscritorio(alfa, (db) =>
      db.notaFiscal.findFirst({ where: { status: "RECUSADA" }, orderBy: { criadoEm: "desc" } })
    );
  ...
  it("sem cadastro fiscal e sem certificado, cada falta tem sua mensagem", async () => {
    const cliente = await comEscritorio(beta, (db) =>
      db.cliente.create({
        data: semEscritorio({ nome: "Cliente do Beta", documento: "52998224725" }),
      })
    );
  ...
  it("cancelamento so depois que a prefeitura aceita", async () => {
    ...
    responder = (caminho) =>
      caminho.includes("/eventos")
        ? { status: 400, json: { erros: [{ descricao: "Fora do prazo de cancelamento." }] } }
    ...
    expect(desempacotar(evento!.corpo.pedidoRegistroEventoXmlGZipB64)).toContain("<Signature");
  });

  it("nota recusada nao se cancela", async () => {
    responder = () => ({ status: 400, json: { erros: [{ descricao: "qualquer erro" }] } });
    await emitirNota(alfa, { ...pedido, clienteId: clienteAlfa }).catch(() => {});

    const recusada = await comEscritorio(alfa, (db) =>
      db.notaFiscal.findFirst({ where: { status: "RECUSADA" }, orderBy: { criadoEm: "desc" } })
    );
    await expect(cancelarNota(alfa, recusada!.id)).rejects.toBeInstanceOf(CancelamentoInvalido);
  });
  ...
  it("nota de um escritorio nao se ve nem se cancela do outro", async () => {
    ...
    await expect(cancelarNota(beta, nota.id)).rejects.toBeInstanceOf(NotaInvalida);
  });
});
