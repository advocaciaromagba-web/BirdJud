...
export class ClienteSemDocumento extends Error {
  ...
  constructor(nome: string) {
    super(`${nome} esta sem CPF/CNPJ, e o Asaas exige o documento para cobrar.`);
...
async function chamarAsaas(
  ...
  if (resposta.status === 401) throw new FalhaNoAsaas("O Asaas recusou a chave de API.");
  if (!resposta.ok) {
    ...
    const descricao =
      Array.isArray(erros) && erros.length > 0 && typeof erros[0] === "object"
        ...
        : "";
    throw new FalhaNoAsaas(descricao || `O Asaas respondeu ${resposta.status}.`);
  }
...
async function chaveDoEscritorio(escritorioId: string): Promise<string> {
  try {
    const dados = await obterIntegracao<{ chave: string }>(escritorioId, "ASAAS");
...
export async function emitirCobranca(
  ...
  if (pedido.valorCentavos <= 0) throw new PedidoInvalido("O valor precisa ser maior que zero.");
  ...
  const cobranca = await comEscritorio(escritorioId, (db) =>
    db.cobranca.create({
      data: semEscritorio({
        ...
        idNoAsaas,
        linkPagamento: typeof criada.invoiceUrl === "string" ? criada.invoiceUrl : null,
        linkBoleto: typeof criada.bankSlipUrl === "string" ? criada.bankSlipUrl : null,
...
export async function sincronizarCobrancas(
  ...
  for (const cobranca of abertas) {
    ...
    try {
      const doAsaas = await chamarAsaas(chave, `/payments/${cobranca.idNoAsaas}`);
      ...
      await comEscritorio(escritorioId, (db) =>
        db.cobranca.update({
          ...
          data: {
            ...
            lancamentoId,
            sincronizadoEm: new Date(),
            ...(pagouAgora ? { pagoEm: quando, valorPagoCentavos: valorPago } : {}),
          },
        })
      );
 ...
 */
export async function cancelarCobranca(escritorioId: string, id: string): Promise<void> {
  const cobranca = await comEscritorio(escritorioId, (db) =>
    db.cobranca.findUnique({ where: { id } })
  );
  if (!cobranca) throw new PedidoInvalido("Cobranca nao encontrada.");
  if (cobranca.status === "PAGA") {
    throw new PedidoInvalido("Cobranca ja paga: o estorno e feito no painel do Asaas.");
  }
  if (cobranca.status === "CANCELADA") return;

  const chave = await chaveDoEscritorio(escritorioId);
  await chamarAsaas(chave, `/payments/${cobranca.idNoAsaas}`, { method: "DELETE" });
