...
export const conectorWhatsapp: Conector = {
  ...
  modulo: "WHATSAPP",
  campos: [
    { nome: "numeroId", rotulo: "ID do numero", tipo: "text", obrigatorio: true },
    { nome: "token", rotulo: "Token de acesso", tipo: "password", obrigatorio: true },
  ],
  resumo: (dados) => `Numero ${dados.numeroId ?? "—"} · token ${mascarar(dados.token)}`,
  ...
  async testar(dados) {
    try {
      ...
      if (!resposta.ok) {
        return { ok: false, detalhe: corpo.error?.message ?? `Meta respondeu ${resposta.status}.` };
      }

      const qualidade = corpo.quality_rating ? ` · qualidade ${corpo.quality_rating}` : "";
      return { ok: true, detalhe: `${corpo.verified_name ?? "Numero conectado"}${qualidade}.` };
    } catch (erro) {
