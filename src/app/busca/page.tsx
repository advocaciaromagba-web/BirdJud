...
import { formatarNumeroProcesso, normalizarNumeroProcesso } from "@/lib/leitura-publicacao";
...
export default async function PaginaBusca({
  ...
  const [modulos, achados] = await Promise.all([
    ...
    termoUtil(termo) ? buscar(contexto.escritorioId, termo) : Promise.resolve([]),
  ]);
          ...
          {achados.map((achado) => (
            <li key={`${achado.tipo}-${achado.id}`} className="py-3">
              <Link href={achado.destino} className="block hover:text-marca">
                ...
                {achado.detalhe ? (
                  <span className="mt-1 block text-sm text-neutral-600">{achado.detalhe}</span>
                ) : null}
              </Link>
            </li>
          ))}
