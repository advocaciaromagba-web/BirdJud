...
import { ListaCobrancas, type CobrancaNaTela } from "@/componentes/ListaCobrancas";
...
export default async function PaginaCobrancas() {
  ...
  const [modulos, dados] = await Promise.all([
    ...
    comEscritorio(contexto.escritorioId, async (db) => ({
      ...
      contaConectada: await db.integracao.count({ where: { tipo: "ASAAS", status: "OK" } }),
    })),
  ]);
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl p-8">
        ...
        <p className="mt-1 text-sm text-neutral-500">
          {emReais(emAberto)} em aberto · cobranca na conta Asaas do proprio escritorio
        </p>
        ...
        {dados.clientes.length > 0 ? (
          <details className="mt-4">
            ...
            <p className="mt-2 text-sm text-neutral-500">
              O Asaas exige CPF/CNPJ do cliente. Cliente sem documento cadastrado e
              recusado com essa mensagem.
            </p>
            <FormularioCriar
              rota="/api/cobrancas"
              campos={[
                {
                  ...
                  opcoes: dados.clientes.map((cliente) => ({
                    valor: cliente.id,
                    rotulo: cliente.documento ? cliente.nome : `${cliente.nome} (sem CPF/CNPJ)`,
                  })),
                },
